...
public class UserServiceImpl implements UserService {
	...
	public void saveUser(UserDto dto) {
		...
		User user = User.builder().empId(empId).name(dto.getName().trim()).email(email)
				.password(passwordEncoder.encode(dto.getPassword().trim())).role(dto.getRole()).department(department)
				.build();
	...
	public UserDto getUserById(Long id) {
		...
		return UserDto.builder().id(user.getId()).empId(user.getEmpId()).name(user.getName()).email(user.getEmail())
				.password(user.getPassword()).role(user.getRole()).departmentId(user.getDepartment().getId())
	...
	public Page<UserDto> getUsers(String keyword, int page, int size) {
		...
		if (keyword != null && !keyword.trim().isEmpty()) {
			users = userRepository.findByNameContainingIgnoreCaseAndIsDeletedFalse(keyword, pageable);
		} else {
			users = userRepository.findByIsDeletedFalse(pageable);
		}

		return users.map(user -> UserDto.builder().id(user.getId()).empId(user.getEmpId()).name(user.getName())
				.email(user.getEmail()).role(user.getRole()).departmentId(user.getDepartment().getId())
				.departmentName(user.getDepartment().getDepartmentName()).build());
	}
}